...
export function PaymentModal({ isOpen, onClose }: PaymentModalProps) {
  ...
  const change = Math.max(
    0,
    (currentMethod === "CASH" ? parseFloat(amountInput) || 0 : 0) -
      remainingBalance
  );
  ...
  const addPayment = () => {
    ...
    if (currentMethod === "MOBILE_MONEY") {
      ...
      details = {
        ...
        change: amount > remainingBalance ? amount - remainingBalance : 0,
      };
    }
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (shiftError || !shiftData || shiftData.status !== "OPEN") {
        ...
        toast.error(
          "Active shift not found or already closed. Please open a new shift."
        );
        return;
      }
    } catch (error) {
    ...
    try {
          ...
          : [
              {
                ...
                details:
                  currentMethod === "CASH"
                    ...
                    ? { network: momoNetwork, reference: momoRef }
                    : { reference: cardRef },
              },
            ];
      ...
      if (saleError) {
        if (saleError.code === "23503") {
          ...
          toast.error(
            "Shift not found. Please open a new shift and try again."
          );
          useShiftStore.getState().setCurrentShift(null);
        }
        throw saleError;
      ...
      for (const p of finalPayments) {
        ...
        if (p.method === "STORE_BALANCE" && customerId) {
          ...
          if (currentBal < p.amount)
            throw new Error("Insufficient store balance");

          await supabase
            .from("customers")
            .update({ store_balance: currentBal - p.amount })
            .eq("id", customerId);
      ...
      if (customerId) {
        ...
        await supabase
          .from("customers")
          .update({ loyalty_points: newPoints, tier })
          .eq("id", customerId);
      }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Split payments or use promo codes
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="mt-2 p-4 bg-primary/5 rounded-xl border border-primary/10">
            ...
            >
              ...
              >
                ...
                {customerId && (
                  <TabsTrigger value="STORE_BALANCE">Balance</TabsTrigger>
                )}
              </TabsList>
              ...
              <div className="space-y-4">
                ...
                <TabsContent value="STORE_BALANCE" className="mt-0">
                  <div className="bg-yellow-50 p-2 rounded border border-yellow-200 text-[10px] text-yellow-800">
                    Using customer's prepaid balance. Ensure they have enough
                    funds.
                  </div>
                </TabsContent>
        ...
        <div className="mt-6 flex gap-3">
          ...
          <Button
            ...
            disabled={
              ...
              (payments.length === 0 && (parseFloat(amountInput) || 0) < total)
            }
          >
            {isLoading ? (
